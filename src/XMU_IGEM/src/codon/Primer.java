...
public class Primer {
	...
	public Primer(String seq)
	{
		...
		this.minPGc = new double[5];
		this.minPTm = new double[5];
		this.minQGc = new double[5];
		this.minQTm = new double[5];
		this.getMin5(K);
	}
	...
	public double[] getMinPGc() {
		return minPGc;
	}

	public double[] getMinPTm() {
		return minPTm;
	}

	public double[] getMinQGc() {
		return minQGc;
	}

	public double[] getMinQTm() {
		return minQTm;
	}
	...
	private boolean getMin5(int k, int l, int m, int n)
	{
		...
		double[] minpgc = this.minPGc;
		double[] minptm = this.minPTm;
		double[] minqgc = this.minQGc;
		double[] minqtm = this.minQTm;
		double b=getSc(cpSeq.substring(k, l),ordSeq.substring(m, n), K)-getScideal(cpSeq.substring(k, l),ordSeq.substring(m, n),K);
		double pgc = getGc(cpSeq.substring(k, l));
		double ptm = getTm(cpSeq.substring(k, l));
		double qgc = getGc(ordSeq.substring(m, n));
		double qtm = getTm(ordSeq.substring(m, n));
