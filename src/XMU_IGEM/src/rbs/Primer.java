...
public class Primer {
	...
	public static void main(String[] args)
	{
		...
		double [] min5=p.getMin5(p.ordSeq, p.cpSeq, K);
		for(int i=0; i<min5.length; i++)
			System.out.print("min["+i+"]="+min5[i]+"\t");
	}
	...
	public Primer(String seq)
	{
		...
		this.K=K;
		//下面这个是输出
		this.min5=this.getMin5(this.ordSeq, this.cpSeq, K);
	}
	...
	//求最小的5个数
	public double[] getMin5(String ordSeq, String cpSeq, double K[])
	{
		double min5[]={Double.MAX_VALUE,Double.MAX_VALUE,Double.MAX_VALUE,Double.MAX_VALUE,Double.MAX_VALUE};
		for(int i=18; i+18<ordSeq.length(); i++)
		{
			for(int j=18; i-j>=0&&j<=28&&(i+j)<=ordSeq.length(); j++)
				min5=getMin5(min5, getSc(ordSeq.substring(i, i+j),cpSeq.substring(i-j, i), K));
		}
		return min5;
	...
	//获取sc_ideal
	public double getScideal(double K[])
	{
		...
		for(int i=18; i<28; i++)
			for(int j=18; j<28; j++)
			{
				cur=K[0]*i+K[1]*j+K[2]*0.55+K[3]*0.55+K[4]*55+K[5]*55;
				if(min>cur)
					min=cur;
			}
		return min;
	...
	private double[] getMin5(double[] a, double b)
	{
		if(b>a[0])
			return a;
		else{
	...
	private double getdeltaHt(String str)
	{
		if(str.length()!=2)
		{
			System.out.println("wrong at primer.getdeltaH");
			return 0.0;
		}
		if(str.equals("AT"))
			return 8.6;
		...
		if(str.equals("GG") || str.equals("CC"))
			return 11.0;
		System.out.println("wrong at primer.getdeltaH, input is "+str);
		return 0.0;
	}
	private double getdeltaSt(String str)
	{
		if(str.length()!=2)
		{
			System.out.println("wrong at primer.getdeltaS");
			return 0.0;
		}
		if(str.equals("AT"))
			return 23.9;
		...
		if(str.equals("GG") || str.equals("CC"))
			return 26.6;
		System.out.println("wrong at primer.getdeltaS. input is "+ str);
		return 0.0;
	}
	...
	private double getdeltaH(String str)
	{
		...
		for(int i=0; i<str.length()-1; i++)
			sum += getdeltaHt(str.substring(i,i+1));
		return sum;
	}
	private double getdeltaS(String str)
	{
		double sum = 0.0;
		for(int i=0; i<str.length()-1; i++)
			sum += getdeltaSt(str.substring(i, i+1));
		return sum;
	}
