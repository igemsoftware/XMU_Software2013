...
public class OptionalPrimer extends JFrame {
	 ...
	 * 
	 */
	private static final long serialVersionUID = -4486856170563547456L;
	...
	public void tovalue() {
		...
		if (t.isChangeSuccess() == true) {
			...
			double[] minpgc = p.getMinPGc();
			double[] minptm = p.getMinPTm();
			double[] minqgc = p.getMinQGc();
			double[] minqtm = p.getMinQTm();
			this.minp = minp;
			this.minq = minq;
			this.minpgc = minpgc;
			this.minptm = minptm;
		...
		public void actionPerformed(ActionEvent arg0) {
			if (arg0.getSource() == add) {
				if (number != -1) {
					...
					double minpgc = OptionalPrimer.this.minpgc[OptionalPrimer.this.number];
					double minptm = OptionalPrimer.this.minptm[OptionalPrimer.this.number];
					double minqgc = OptionalPrimer.this.minqgc[OptionalPrimer.this.number];
					double minqtm = OptionalPrimer.this.minqtm[OptionalPrimer.this.number];
					new SelectOne(p, q, minpgc, minptm, minqgc, minqtm);
				}
			}
	...
	class OptionalPrimerImage extends JPanel {
		 ...
		 * 
		 */
		private static final long serialVersionUID = 7727694933727988515L;
