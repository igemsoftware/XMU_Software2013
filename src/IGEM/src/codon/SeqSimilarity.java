...
public class SeqSimilarity {
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < sigma.length; i++) {
			SeqSimilarity s = new SeqSimilarity(sigma[i], seq);
	...
	private double getGS(int spacemin, int spacemax, int d, int center,
			double adjuste) {
		double n = 0.0;
		for (int i = spacemin; i <= spacemax; i++) {
			n += getAccessibility(spacemin, spacemax, i, center);
		}
		return -Math.log(getAccessibility(spacemin, spacemax, d, center) / n)
				/ Math.log(2.0) + adjuste;
	}
	...
	private boolean getMaxSigma(String seq)
	{
		...
		for(int i=0; i<sigma.length; i++)
		{
			this.getSimilarity(sigma[i].getMatrix1(), sigma[i].getMatrix2(), seq, sigma[i].getMotifWeight(), sigma[i].getSpaceWeight(), sigma[i].getSpaceMin(), sigma[i].getSpaceMax(), (sigma[i].getSpaceMin()+sigma[i].getSpaceMax())/2, sigma[i].getAdjustE());
			cur = this.Similarity;
			if(max<cur)
			{
